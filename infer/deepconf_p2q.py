...
class DeepConfInference:
    ...
    def process_trace(self, choice, trace_id):
        ...
        # 计算置信度
        confs = self.compute_confidence([t.top_logprobs for t in choice.logprobs.content])
    ...
    def inference(self, messages, verbose=False):
        ...
        for j in range(len(responses.choices)):
            trace_data = self.process_trace(responses.choices[j], self.warmup_traces + j)
...
def request_model(query_list=None, answer_list0=None, time_str=None, location=None, all_p2q_res=False, temperature=TEMPERATURE, confidence_percentile=CONFIDENCE_PERCENTILE, window_size=WINDOW_SIZE, warmup_traces=WARMUP_TRACES, total_budget=TOTAL_BUDGET):
    ...
    for query, answer in zip(query_list, answer_list):
        user = PROMPT["user"].format(ans=answer[:200]+answer[200:][-200:], query=query)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--csv-path', '-i', type=str, required=True, 
